...
from BTResNet import resnet18, resnet34, resnet50, resnet101, resnet152

from utils.tools import *
from utils.loss import AAMsoftmax

class TBReNetModel(nn.Module):
    def __init__(self, attention, stats, lr, lr_decay, n_class, m, s, test_step, **kwargs):
        super(TBReNetModel, self).__init__()
        
        self.speaker_encoder = resnet18(attention, stats).cuda()
    ...
    def eval_scores(self, eval_list, eval_path, score_path):
        self.eval()
        files = []
        embeddings = {}
        lines = open(eval_list).read().splitlines()
